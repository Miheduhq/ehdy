...
const Header: React.FC<HeaderProps> = ({ isMenuOpen, setIsMenuOpen, openContactModal }) => {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/80 backdrop-blur-sm border-b border-gray-100 animate-slide-down">
        ...
        <div className="flex items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center space-x-6 xl:space-x-8 animate-fade-in">
            <button 
              ...
              className="text-gray-700 hover:text-purple-600 transition-all duration-300 text-sm xl:text-base font-medium hover:scale-110 transform"
            >
              Accueil
            </button>
            <button 
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 lg:space-x-4 animate-fade-in-right">
            ...
            <button 
              onClick={handleContactClick}
              className="hidden md:block bg-gradient-to-r from-purple-600 to-pink-500 text-white px-3 lg:px-4 xl:px-6 py-2 rounded-full font-medium hover:from-purple-700 hover:to-pink-600 transition-all duration-300 text-xs lg:text-sm xl:text-base whitespace-nowrap hover:scale-105 transform hover:shadow-lg"
            >
              Devis gratuit
            </button>
            
            {/* Hand logo in white circle */}
            <div className="w-12 h-12 sm:w-14 sm:h-14 lg:w-12 lg:h-12 bg-gradient-to-r from-purple-100 to-pink-100 rounded-full flex items-center justify-center flex-shrink-0 hover:scale-110 transition-transform duration-300 animate-bounce-gentle">
              <img 
            ...
            {/* Mobile menu button */}
            <button 
              className="lg:hidden text-gray-700 p-1 hover:bg-gray-100 rounded-md transition-all duration-300 hover:scale-110 transform"
          ...
          <div className="lg:hidden animate-slide-down">
            <div className="px-2 pt-4 pb-6 space-y-1 bg-white/90 backdrop-blur-md rounded-lg mt-4 border border-gray-200">
              <nav className="flex flex-col space-y-3">
                <button 
                  onClick={() => scrollToSection('accueil')}
                  className="text-gray-700 hover:text-purple-600 transition-all duration-300 px-3 py-2 rounded-md text-base font-medium hover:bg-gray-50 text-left hover:scale-105 transform"
                >
                  Accueil
                </button>
                <button 
                ...
                <div className="pt-3 border-t border-gray-200">
                  <button 
                    onClick={handleContactClick}
                    className="w-full bg-gradient-to-r from-purple-600 to-pink-500 text-white px-4 py-3 rounded-lg font-medium hover:from-purple-700 hover:to-pink-600 transition-all duration-300 text-sm hover:scale-105 transform hover:shadow-lg"
                  >
                    Devis gratuit
                  </button>
                </div>
